...
web_ui_log_path="${WEB_UI_LOG_PATH:-${runtime_logs_dir}/web-ui.stdout.log}"
...
python scripts/run_remote_workbench_service.py web-ui --config "${config_path}" >>"${web_ui_log_path}" 2>&1 &
web_ui_pid=$!

cleanup() {
    for pid in "${web_ui_pid:-}" "${job_api_pid:-}"; do
...
echo "Web UI log: ${web_ui_log_path}"

wait -n "${job_api_pid}" "${web_ui_pid}"
