...
7. **"恢复默认"按钮保留 hidden clone 字段**（PR #13 Codex P2）：点击
   ``恢复默认`` 不能直接 ``setSettings(DEFAULT_SETTINGS)``，必须用
   ``setSettings((s) => ({ ...DEFAULT_SETTINGS, <6 个 clone 字段从 s 透传> }))``
   —— 否则 reset + save 会覆盖后端 worker_enabled / allowlist / 配额。
...
# ---------------------------------------------------------------------------
# 7. "恢复默认" 按钮必须保留全部 6 个 cosyvoice_clone_* 字段
#    （PR #13 Codex P2 fix — discussion 2026-05-27 02:52:13Z）
# ---------------------------------------------------------------------------


def test_admin_settings_reset_button_preserves_hidden_clone_fields():
    """**D.1 守卫 7（PR #13 Codex P2 fix）**：``恢复默认`` 按钮必须从 current
    state 透传**全部 6 个** ``cosyvoice_clone_*`` 字段，不能直接
    ``setSettings(DEFAULT_SETTINGS)`` 覆盖。

    背景：P1 fix 把全部 6 个 clone 字段塞进 page state 但只渲染 GA toggle
    一个 UI 控件。如果"恢复默认"是 ``setSettings(DEFAULT_SETTINGS)`` 直接覆
    盖，admin 点击后 worker_enabled / allowlist / max_voices_per_user 等都
    会被覆盖为默认值。下次点"保存设置" 时 full-body POST 把这些值写回后端
    —— 等于 "点恢复默认 + 保存" 静默关掉克隆 worker / 擦掉 beta 用户。这是
    P1 同源的 fail-open path。

    正确实现：``setSettings((s) => ({ ...DEFAULT_SETTINGS, <6 个 clone 字段
    透传> }))`` —— 把 visible 字段 reset 成默认，但把 hidden clone state
    从 prev state 带过来。

    Codex thread:
    https://github.com/sun9bear/AITransVideo/pull/13#discussion_r3308123420
    """
    src = _read_page()
    ...
    # 我自己的 onClick 长注释里写了 ``setSettings(DEFAULT_SETTINGS)`` 作为反例说明，
    # 不剥掉的话 forbidden_pattern 会误报。re.MULTILINE + ``^`` 处理多行注释。
    ...
    assert not forbidden_pattern.search(onclick_window), (
        ...
        "fields —— 必须用 setSettings((s) => ({ ...DEFAULT_SETTINGS, <6 个 "
        "clone 字段透传> })) 模式。详见 PR #13 Codex P2 discussion_r3308123420。"
    )
    ...
    # 4. 必须显式透传**全部 6 个** clone 字段（任何一个漏 = 被 reset）
    for field_name in CLONE_INTERFACE_FIELDS.keys():
        # 匹配 ``<field>: s.<field>`` 或 ``<field>: s.<field>,``
        ...
        assert preserve_pattern.search(onclick_window), (
            ...
            f"透传。漏掉任何一个都会让 reset 把后端配置覆盖为默认值。"
