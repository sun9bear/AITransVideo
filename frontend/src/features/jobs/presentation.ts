...
function buildNativeReviewRoute(stage: ReviewStage | null, jobId: string | null) {
  ...
  if (stage === 'speaker_review') {
    return `/reviews/${jobId}/speaker`
  }

  if (stage === 'translation_config_review') {
    return `/reviews/${jobId}/translation-config`
  }

  if (stage === 'translation_review') {
    return `/reviews/${jobId}/translation`
  }

  if (stage === 'voice_review') {
    return `/reviews/${jobId}/voice`
  }
