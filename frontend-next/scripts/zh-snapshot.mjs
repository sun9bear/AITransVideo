...
// 16) CM-04（support 客服组件英文化）：appSupport namespace 的内联中文迁入字典后，默认 zh 值
//     必须与改造前 SupportLauncher/SupportConversationPanel/SupportMessageList/
//     SupportHandoffBanner/support-copy.ts 源【逐字节】相同（红线 R1）。范围=用户可见 support
//     组件；admin/（HandoffTicketsPanel/PresenceConfigCard/WeChatQrCard）+
//     AdminPresenceSwitcher.tsx + useAdminHeartbeat.ts 明确豁免（operator-only，不译，不在此
//     namespace）。钉死间隔号 ·(U+00B7)/全角冒号：/全角省略号 …/✦ U+2726 最敏感的代表串：
const zhSup = JSON.parse(readFileSync(path.join(root, "messages/zh/appSupport.json"), "utf8"))
assert.equal(zhSup.labels.launcherLabel, "客服", "appSupport.labels.launcherLabel 漂移（红线 R1）")
assert.equal(zhSup.labels.launcherTooltip, "在线客服 / 帮助", "appSupport.labels.launcherTooltip 半角斜杠漂移（红线 R1）")
assert.equal(zhSup.labels.panelTitle, "客服小助手", "appSupport.labels.panelTitle 漂移（红线 R1）")
assert.equal(zhSup.labels.panelSubtitle, "AI 优先回答 · 解决不了会转人工", "appSupport.labels.panelSubtitle 间隔号 · 漂移（红线 R1）")
assert.equal(zhSup.labels.inputPlaceholder, "在这里描述你的问题…", "appSupport.labels.inputPlaceholder 全角省略号 … 漂移（红线 R1）")
assert.equal(zhSup.labels.sendButton, "发送", "appSupport.labels.sendButton 漂移（红线 R1）")
assert.equal(zhSup.labels.resolvedButton, "已解决", "appSupport.labels.resolvedButton 漂移（红线 R1）")
assert.equal(zhSup.labels.notResolvedButton, "没解决，转人工", "appSupport.labels.notResolvedButton 全角逗号漂移（红线 R1）")
assert.equal(zhSup.labels.closeButton, "关闭", "appSupport.labels.closeButton 漂移（红线 R1）")
assert.equal(zhSup.labels.handoffCreatedNote, "已创建人工工单", "appSupport.labels.handoffCreatedNote 漂移（红线 R1）")
assert.equal(zhSup.labels.handoffFailedNote, "转人工失败，请稍后再试或直接发邮件", "appSupport.labels.handoffFailedNote 全角逗号漂移（红线 R1）")
assert.equal(zhSup.labels.handoffWaitingNote, "已转人工，运营会通过邮件回复你", "appSupport.labels.handoffWaitingNote 全角逗号漂移（红线 R1）")
assert.equal(zhSup.labels.unauthHelp, "登录后可以让客服读取你的任务和账单上下文。", "appSupport.labels.unauthHelp 漂移（红线 R1）")
assert.equal(zhSup.labels.budgetExhaustedNote, "AI 客服繁忙，已切换到模板回复。", "appSupport.labels.budgetExhaustedNote 全角逗号漂移（红线 R1）")
assert.equal(zhSup.fallback.greeting, "你好，我可以帮你解答试用、套餐、导出剪映草稿和任务排障问题。", "appSupport.fallback.greeting 顿号/全角逗号漂移（红线 R1）")
assert.deepEqual(
  zhSup.fallback.quickQuestions,
  ["试用会自动扣费吗？", "怎么导出剪映草稿？", "任务失败怎么办？", "我要找人工客服"],
  "appSupport.fallback.quickQuestions 全角问号/顺序漂移（红线 R1）",
)
assert.equal(zhSup.conversation.createFailed, "创建会话失败", "appSupport.conversation.createFailed 漂移（红线 R1）")
assert.equal(zhSup.conversation.sendFailed, "发送失败", "appSupport.conversation.sendFailed 漂移（红线 R1）")
assert.equal(zhSup.conversation.handoffFailed, "转人工失败", "appSupport.conversation.handoffFailed 漂移（红线 R1）")
assert.equal(zhSup.conversation.dialogAriaLabel, "客服对话", "appSupport.conversation.dialogAriaLabel 漂移（红线 R1）")
assert.equal(zhSup.conversation.onlineLabel, "客服在线", "appSupport.conversation.onlineLabel 漂移（红线 R1）")
assert.equal(zhSup.conversation.offlineLabel, "运营离线", "appSupport.conversation.offlineLabel 漂移（红线 R1）")
assert.equal(zhSup.conversation.restoredNotice, "已恢复你之前的客服对话", "appSupport.conversation.restoredNotice 漂移（红线 R1）")
assert.equal(
  zhSup.conversation.offlineMessageDefault,
  "运营暂未在线，可扫码添加客服微信，我们尽快回复。",
  "appSupport.conversation.offlineMessageDefault 全角逗号漂移（红线 R1）",
)
assert.equal(zhSup.conversation.offlineQrAlt, "客服微信二维码", "appSupport.conversation.offlineQrAlt 漂移（红线 R1）")
assert.equal(zhSup.conversation.offlineInputPlaceholder, "请在微信继续沟通", "appSupport.conversation.offlineInputPlaceholder 漂移（红线 R1）")
assert.equal(zhSup.messageList.emptyHint, "还没有对话记录。问点什么吧。", "appSupport.messageList.emptyHint 全角句号漂移（红线 R1）")
assert.equal(zhSup.messageList.humanBadge, "✦ 客服", "appSupport.messageList.humanBadge ✦ U+2726 漂移（红线 R1）")
assert.equal(zhSup.messageList.thinking, "AI 正在思考…", "appSupport.messageList.thinking 全角省略号 … 漂移（红线 R1）")
assert.equal(
  zhSup.handoffBanner.recommended,
  "建议转人工客服处理。点下方「转人工」按钮即可。",
  "appSupport.handoffBanner.recommended 全角直角引号「」漂移（红线 R1）",
)
assert.equal(zhSup.handoffBanner.createdPrefix, "：", "appSupport.handoffBanner.createdPrefix 全角冒号漂移（红线 R1，拼接 handoffCreatedNote+此+handoffWaitingNote）")
assert.equal(
  zhSup.handoffBanner.closed,
  "工单已关闭，如需继续咨询可开新对话。",
  "appSupport.handoffBanner.closed 全角逗号漂移（红线 R1）",
)

console.log("[zh-snapshot] OK — 默认 zh 不变量 + site.ts inert + auth/marketing/seo/app/errors/工作台 W1+W2a+W2b+W3a+W3b+W4a+W4b/CM-04 support 字节一致 + en seo 双源同步 全部通过")
