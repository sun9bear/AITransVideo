...
// localizedRoutes 必须非空（含已翻旗页 /pricing /trial；home `/` 暂移出待 UI-03g，见 site.ts）。
...
// home 路由：暂未翻旗（NEXT_PUBLIC_ENABLE_ANONYMOUS_PREVIEW=1 时 AnonymousTrialPanel /en 泄漏中文，
// 待 UI-03g）→ NO en 键。防回归：未配套本地化 panel 前把 "/" 加回 localizedRoutes 会在此 red
// （@codex #66 P2 + 项目主 2026-06-27 决策）。UI-03g 落地时同步移除本断言 + 把 "/" 加回上列。
const homeRoute = "/"
assert.ok(
  !routes.includes(homeRoute),
  `${homeRoute}（home）暂不应在 localizedRoutes（待 UI-03g 本地化 AnonymousTrialPanel；@codex #66 P2）`,
)
const hlHome = site.hreflangLanguages(homeRoute)
if ("en" in hlHome) {
  fail(`${homeRoute}（home）: 不应含 en 键（home 暂未翻旗，hreflang=${JSON.stringify(hlHome)}）`)
}
...
console.log(
  `[hreflang-check] OK — ${routes.length} 条 localizedRoute 互惠/自指/x-default 全通过；legal(${legalRoute}) + home(${homeRoute}) 无 en`,
)
