      ...
      await approveTranslationReview({
        ...
        segments: Object.fromEntries(
          resource.items.map((item) => {
            ...
            return [item.segmentId, { cnText: current.cnText, rewriteRequested: current.rewriteRequested, translationConfirmed: current.translationConfirmed, updatedAt: current.updatedAt || now }]
          }),
        ),
      })
      onAdvanced()
                  ...
                  <div className="group rounded-xl border border-border bg-muted/30 transition hover:border-primary/30 hover:bg-primary/5 focus-within:border-primary/40 focus-within:bg-primary/5" style={{ borderLeft: "3px solid var(--ultramarine)" }}>
                    <textarea
                      ...
                      onChange={(event) => {
                        ...
                        setSegments((s) => ({ ...s, [item.segmentId]: { ...(s[item.segmentId] ?? current), cnText: v, updatedAt: new Date().toISOString() } }))
                      }}
