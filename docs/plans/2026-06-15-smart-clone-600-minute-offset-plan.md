...
**状态：** IMPLEMENTED v1.5（TDD 实现已落 worktree `claude/anon-clone-enable` 未提交；内部 5-lens 对抗审查 0 must-fix；CodeX 生产审计 78/100「可进 PR/继续审，翻旗激活前补 3 项」全已回应） / **下一步：CodeX 复审修订 → 项目主部署（先满足激活前 gate）**
...
> - **激活前 gate（CodeX 建议，项目主把关）**：alembic 037/038/039 在目标环境跑完 + cost_summary 审计字段已补 ✓ + 并发 convert 压测或 advisory lock + 真金小额 E2E 覆盖 preview→full 与 full smart auto-clone，**之后**才翻 `smart_preview_clone_enabled`。默认 OFF inert 可先合 main。
...
- **后备防线**：PG `job_id` 主键唯一（防真落两行）。真正并发双击的极小窗口（两 POST 都在对方 commit 前 pre-check）可在实现期加 advisory lock 串行化 check+forward（镜像 P3e-4b `_acquire_global_cap_lock`）——本期默认实现先靠 gateway pre-check + PK，advisory lock 视压测决定。
...
4. 实现：
   ...
   - **D-C #3**：convert 确定性 `_convert_job_id` + forward 前 existing-check（Option-C 预供）→ best-effort 单飞（账本 single-use 兜底钱-正确；advisory lock 翻旗激活前补）。
