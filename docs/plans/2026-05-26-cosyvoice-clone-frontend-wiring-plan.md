...
2. ✅ DB `user_voices` 新行 `provider="cosyvoice_voice_clone"`、`voice_id="cosyvoice-v3.5-*"` 格式、**默认 `is_temporary=TRUE, temporary_expires_at=now+7d, expired_at=NULL`**（见 §12；注意 `temporary_expires_at` 是新增字段，与现有 `expired_at`【软删时间戳】严格区分）
...
| Grouping | 包含 | 显示规则 |
|---|---|---|
| **本任务临时克隆** | `is_temporary=TRUE` **且** `source_job_id == current_jobId` **且** `expired_at IS NULL` **且** `temporary_expires_at > now()` | 仅在该 voice 所属任务里显示；voice option 文字后带"临时·剩余 N 天"标签 |
| **我的 CosyVoice 克隆音色** | `is_temporary=FALSE` **且** `expired_at IS NULL`（用户主动保存的） | 跨任务全局显示 |

**两个 `*expires*` / `*expired*` 字段的语义严格分离**（v4-followup 规范，避免误用）：

- **`expired_at`**：**现有字段**，所有 voice 通用，表示**实际软删除/失效时间戳**（NULL = active）。所有 active 查询（dropdown、routing lookup、admin 列表、quota 计算等）**必须用** `expired_at IS NULL` 作为唯一 active 判断标准
- **`temporary_expires_at`**：**新增字段**，**仅** `is_temporary=TRUE` 时使用，表示**临时音色计划过期时间**。**绝不**用 `temporary_expires_at` 来判断 active —— 它只是清理任务的入选条件之一
...
```tsx
{/* 1. 本任务临时克隆（active + is_temporary=TRUE + jobId 匹配 + 未到计划过期） */}
  ...
  <optgroup label={`本任务临时克隆 (${cosyvoiceJobTempVoices.length})`}>
    {cosyvoiceJobTempVoices.map((v) => (
      <option key={v.voiceId} value={v.voiceId}>
        {v.label} · 临时·剩余 {daysUntil(v.temporaryExpiresAt)} 天
      </option>
    ))}
  </optgroup>
) : null}

{/* 2. 长期保存的克隆（active + is_temporary=FALSE） */}
...
两个集合的过滤逻辑（**全部以 `expiredAt === null` 作为 active 判断标准**，与现有所有 voice active 查询保持一致）：

- `cosyvoiceJobTempVoices`：`getUserVoices()` filter
  ```
  v.provider === 'cosyvoice_voice_clone'
  && v.expiredAt === null               // active（必备）
  && v.isTemporary === true             // 临时音色
  && v.sourceJobId === jobId            // 限本任务
  && new Date(v.temporaryExpiresAt) > new Date()   // 计划未过期（清理 sweeper 滞后兜底）
  ```
- `cosyvoiceSavedVoices`：`getUserVoices()` filter
  ```
  v.provider === 'cosyvoice_voice_clone'
  && v.expiredAt === null               // active
  && v.isTemporary === false            // 长期保存
  ```
...
|---|---|---|---|---|---|
| **任务级临时音色**（默认） | 用户在 CosyVoiceCloneModal 完成克隆，不勾"保存到我的音色库" | `user_voices` 写入一行：`is_temporary=true, temporary_expires_at=now+7d, expired_at=NULL` | 当前任务的音色选择 dropdown 显示，**带"临时·剩余 N 天"标签**；**不**显示在长期"我的 CosyVoice 克隆音色"分组 | 保留 voice_id 直到 `temporary_expires_at`；过期后清理任务 delete remote + 本地软删（写 expired_at） | 7 天 |
| **长期保留音色** | 用户在 modal 勾"保存到我的音色库"（默认 unchecked） | `user_voices` 写入一行：`is_temporary=false, temporary_expires_at=NULL, expired_at=NULL` | 立即出现在长期"我的 CosyVoice 克隆音色"分组，跨任务可见 | 永久保留（直到用户手动从音色库删，触发软删 = 写 expired_at） | 无限 |
...
Phase 4.1 已落 `user_voices` 表（含现有 **`expired_at`** 字段，全表通用软删时间戳），本节决策**新增 2 列，不动 `expired_at`**：

```sql
ALTER TABLE user_voices
  ADD COLUMN is_temporary BOOLEAN NOT NULL DEFAULT FALSE,
  -- ⚠️ 字段命名必须是 temporary_expires_at（不要简写成 expires_at）：
  --   现有 expired_at = 已软删时间戳（所有 active 查询的判据）
  --   新增 temporary_expires_at = 临时音色计划过期时间
  --   两个字面太像，简写成 expires_at 会让后续 query 大概率写错
  ADD COLUMN temporary_expires_at TIMESTAMP WITH TIME ZONE NULL;

-- Partial index on temporary_expires_at；只索引 is_temporary=TRUE
-- 且尚未软删（expired_at IS NULL）的行，给清理 sweeper 用
CREATE INDEX idx_user_voices_temp_expires_pending
  ON user_voices (temporary_expires_at)
  WHERE is_temporary = TRUE AND expired_at IS NULL;
```

**`expired_at` vs `temporary_expires_at` 语义合同**（**任何实施者必须先看完这段再写 query**）：

| 字段 | 来源 | 语义 | 用于判断 active | 用于清理 |
|---|---|---|---|---|
| `expired_at`（现有） | Phase 4.1 已落，全表通用 | **实际软删时间戳**，NULL = active；写入即"逻辑删除完成" | ✅ **唯一 active 判据** —— 所有 dropdown、routing、quota、admin 列表查询都用 `WHERE expired_at IS NULL` | ❌ 不是 sweeper 入选条件，是 sweeper 的**输出**（sweeper 成功删 DashScope 后写它） |
| `temporary_expires_at`（新增） | v4-followup Phase A 落地 | **临时音色计划过期时间**；仅 `is_temporary=TRUE` 时使用 | ❌ **绝不**用它判断 active；只是 sweeper 入选时刻 | ✅ sweeper 用 `WHERE is_temporary=TRUE AND temporary_expires_at < now() AND expired_at IS NULL` 选行 |

写到 Phase 4.2 Phase A 后端实施清单。Phase 4.1 已落 voice 行默认 `is_temporary=FALSE, temporary_expires_at=NULL, expired_at` 保持原值，不影响向后兼容（旧 MiniMax 长期音色行为不变）。

### 12.4 过期清理任务（v4-followup：软删，不 hard delete）

新增 daily cron / scheduled task（建议时间窗：UTC 04:00，避开用户活跃高峰）：

**入选 SQL**（用 §12.3 partial index）：

```sql
SELECT id, voice_id
FROM user_voices
WHERE provider = 'cosyvoice_voice_clone'   -- 只动 CosyVoice 路径，不误伤 MiniMax
  AND is_temporary = TRUE                  -- 只动临时音色，不删长期保存
  AND temporary_expires_at < now()         -- 计划过期
  AND expired_at IS NULL                   -- 尚未软删（防重复处理 + 防误删历史归档）
ORDER BY temporary_expires_at ASC
LIMIT 200;                                 -- 单批 cap，防 DashScope 限流
```

**每行执行**：

1. 调 DashScope `delete_voice(voice_id)` 删远端（请求 timeout 30s + 重试 3 次指数退避）
2. **远端成功** → `UPDATE user_voices SET expired_at = now() WHERE id = :id`（**软删**，保留行）
3. **远端失败** → 不写 `expired_at`，等下一轮重试；连续失败 ≥ 3 轮 → 写入 admin 告警 audit 行

**为什么软删而不是 hard delete**：

- **审计**：留 `provider / target_model / source_job_id / clone_sample_segment_ids / temporary_expires_at` 给未来排障 / 用户问"我的这条克隆为啥不见了"
- **避免 FK 级联**：`user_voices.id` 可能被 `dubbing_tasks` / `voice_routing_log` / 其他表外键引用，hard delete 触发 cascade 风险
- **可恢复**：万一 sweeper 误删（bug / 误配 admin policy），写了 `expired_at` 还能手动 `UPDATE expired_at = NULL` 恢复 DB；hard delete 后只能从备份恢复
- **统计口径稳定**：管理后台"历史克隆次数 / 月活克隆用户数"等指标基于行存在 + status filter，软删让指标计算简单

**幂等保证**：sweeper 选行条件含 `expired_at IS NULL`，重跑同 voice 会被 partial index 直接跳过，**绝不会**对 DashScope 同一 voice_id 重复发 delete 请求。

**关键约束**：

- 清理任务**只动** `provider = 'cosyvoice_voice_clone'` —— 跨 provider 误删 MiniMax voice 是 critical bug
- 清理任务**只动** `is_temporary = TRUE` —— 误删长期保存音色是 critical bug
- §6 守卫测试必须包含：构造混合 row（MiniMax/CosyVoice × 临时/长期 × 过期/未过期）输入，验证 sweeper 只动正确组合
- 长期音色用户主动删除走另一条路径（`DELETE /gateway/user-voices/{id}` endpoint），那条 endpoint **也用软删**（写 `expired_at = now()`），与 sweeper 保持一致
...
- **克隆完成提示**：
  ...
  - 长期："✅ 克隆成功 — 已保存到我的音色库"
- **过期后用户在另一个任务想用这个音色**：dropdown 不显示（已软删 → `expired_at` 已写值 → active 查询过滤掉）；用户记得克隆过 → 提示"该音色已过期，请重新克隆"，**绝不静默重克隆**（CLAUDE.md 付费 API 硬约束）
- **regenerate 跨过 `temporary_expires_at` 边界**：sweeper 选行前，**先扫**当天有 `regenerate_tts` event 引用该 voice_id 的 job，**延 `temporary_expires_at` 7 天**（**只动 `temporary_expires_at`，绝不碰 `expired_at`**）。避免用户编辑中途音色被清。延期写入 SQL：
  ```sql
  UPDATE user_voices
     SET temporary_expires_at = GREATEST(temporary_expires_at, now() + interval '7 days')
   WHERE id = :id
     AND is_temporary = TRUE
     AND expired_at IS NULL;     -- 已软删的不再续命
  ```
...
| temporary_expires_at 延期 hook（regenerate event listener） | +60 | 1-2h |
...
- [x] **§12 新增**：克隆音色保存策略（任务级临时 7d 默认 + 用户主动保存进长期音色库；schema 加 `is_temporary` + `temporary_expires_at` 列 + partial index；过期清理 scheduled task；regenerate 延 TTL；用户体验细节）
...
### v4-followup（Codex 2026-05-26 字段命名 + 软删 review fix）

- [x] **字段重命名**：`expires_at` → `temporary_expires_at`（避免与现有 `user_voices.expired_at` 软删时间戳字面冲突；前者**绝不**用于 active 判断，仅作清理 sweeper 入选条件）
- [x] **新增语义合同表**（§12.3）：明确两字段职责分离 + active 查询合同 `WHERE expired_at IS NULL` 是唯一标准
- [x] **partial index 重命名 + 缩小条件**：`idx_user_voices_temp_expires_pending`，仅索引 `is_temporary=TRUE AND expired_at IS NULL` 的行
- [x] **清理任务改软删**（§12.4 重写）：成功 delete DashScope 后 `UPDATE expired_at = now()`（不 hard delete），保留审计 + 避免 FK 级联 + 可恢复
- [x] **regenerate 续命 SQL**（§12.5）：明确 `UPDATE temporary_expires_at`（**绝不动** `expired_at`），加 `AND expired_at IS NULL` 防对已软删行延期
- [x] **dropdown filter 加强**（§3.5 / §5.4）：`cosyvoiceJobTempVoices` 4 条 AND 条件含 `expiredAt === null` 是 active 判据 + `temporaryExpiresAt > now()` 是 sweeper 滞后兜底
- [x] **DoD #2**（§1.3）：写明 `is_temporary=TRUE, temporary_expires_at=now+7d, expired_at=NULL` 三字段初始值
- [x] **§12.6 工作量表 + §11 记录**：字段名同步更新
