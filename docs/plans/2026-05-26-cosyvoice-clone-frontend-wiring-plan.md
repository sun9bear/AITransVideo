...
- **G3**：克隆成功的 voice 显示在 CosyVoice tab 下拉的「我的 CosyVoice 克隆」grouping
...
2. ✅ DB `user_voices` 新行 `provider="cosyvoice_voice_clone"`、`voice_id="cosyvoice-v3.5-*"` 格式
3. ✅ MiniMax tab + "克隆音色"按钮 → `POST /jobs/{id}/voice-clone` 仍 200 → 武汉 worker audit JSONL **不** 出现新条目
4. ✅ Smart 版任务自动 clone → `provider="minimax_voice_clone"` 仍正常注册（CI 守卫 + 手动 smoke）
5. ✅ 错样本（≤3s / >60s / 非 PCM16 WAV / 总时长越界）→ **前端禁用"开始克隆"按钮 + 红字提示**；用户绕过前端（直接打 endpoint）时**后端 400** 早 fail，**绝不打武汉 worker**（守 worker 调用预算 + DashScope 额度）
6. ✅ 跨 phase 回归测全绿 + 新增 5 条以上守卫测试
...
### 3.5 决策 E：克隆音色在下拉的归类 —— 独立 grouping ✅

**问题**：克隆出来的 CosyVoice voice 该怎么显示在 CosyVoice tab 下拉里？

**候选**：
- **E1**：混在 CosyVoice 现有的「女声 / 男声 / 其他」grouping 里。缺点：用户自己的克隆和官方预设音色混杂。
- **E2** ✅：**新增 grouping「**🎤 我的 CosyVoice 克隆**」放在最顶端**。优点：用户找自己刚克隆的音色一眼看到。

**选 E2**。同样 MiniMax tab 下「我的 MiniMax 克隆」也保持现有逻辑（不动）。
...
**改 3：dropdown 添加「🎤 我的 CosyVoice 克隆」grouping**（约 953-968 行附近）：

在现有 dropdown 的 `optgroup` 前面加一组：

```tsx
{currentProvider === 'cosyvoice' && cosyvoiceUserVoices.length > 0 ? (
  <optgroup label={`🎤 我的 CosyVoice 克隆 (${cosyvoiceUserVoices.length})`}>
    {cosyvoiceUserVoices.map((v) => (
...
`cosyvoiceUserVoices` 从 `getUserVoices()` 过滤 `v.provider === 'cosyvoice_voice_clone'` 得出。
...
1. **target_model 默认值**：本 plan 选 `cosyvoice-v3.5-flash`。需要确认 plus 模型是否在 Phase 4.2 直接开放给所有用户，还是先 admin only？
2. **克隆音色 grouping 命名**：「🎤 我的 CosyVoice 克隆」or「我的克隆音色 (CosyVoice)」or「克隆音色 (个人)」？语言学/营销决策。
3. **consent modal 法律文案**：用 Phase 4.1 plan §授权文案 v1 现成的？还是要请律师再 review？（**强烈建议**：本 plan 范围内沿用现成文案，律师 review 单独跑）
4. **Smart 版要不要也接 CosyVoice 自动克隆**？目前 Phase 4.2 范围**只做 Studio 版人工 review 路径**。Smart 自动 clone 仍走 MiniMax。如果将来 Smart 也要支持 CosyVoice，是 Phase 4.3 的事。
5. **价格显示**：CosyVoice 克隆 cost 用现有 `voice_clone_cost_credits` pricing 字段还是单独定价？（建议沿用，避免引入价格分级复杂度）
