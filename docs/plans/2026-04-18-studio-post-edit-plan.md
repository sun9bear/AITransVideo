...
- 修订日期：2026-04-25（v3，任务命名与 S2 中文标题落地验证）
- 作者：sun9bear（Claude Opus 4.7 协助起草与修订）
- 状态：**Phase 0 部分已上线；任务命名 / display_name / expires_at 修复已部署美国主机并验证；post-edit 主流程仍待 Phase 1**
- 依赖方案：
...
| D10 | 任务名规则 | 新建 YouTube 任务先用 `油管视频 YYYY-MM-DD NNN`；上传任务若文件名含中文则取文件名主干，否则用 `上传视频 YYYY-MM-DD NNN`；S2 审校 Pass 2 生成 `display_title_zh` 后自动替换自动名；用户手动重命名不被覆盖；重名后缀 `_xxxx` |
...
**Migration 回填规则**：
- `display_name`：创建新任务时用 §6.2 初始自动命名算法；历史数据回填可用 `getJobDisplayTitle` 等价算法兜底，后续 S2 可替换为内容感知中文标题
...
- 初始占位名：YouTube 使用 `油管视频 YYYY-MM-DD NNN`；上传无合适中文文件名时使用 `上传视频 YYYY-MM-DD NNN`
- 中文文件名主干：等效显示宽度 24（英文 1 / 中日韩 2），超出截断
- S2 内容感知中文标题：Pass 2 输出 `display_title_zh`，要求中文、8-24 个汉字为佳；服务端清洗后最多保留 60 字符
- 重名后缀：`_` + 4 位随机（小写字母 + 数字），显示宽度固定 5
- 自动名总显示宽度建议：主体 24 + 后缀 5 = 29
- DB `VARCHAR(60)` 留冗余（S2 标题 / 用户手改 / 副本后缀可能超过自动名宽度）
...
1. YouTube 源（创建任务时）：
   "油管视频 YYYY-MM-DD NNN"
   - yt-dlp title 只保存为 source title / 探测元数据，不作为卡片主标题
   - NNN = 该用户当天 YouTube 占位名顺序编号（001-999 三位）
...
3. 本地上传，但没有合适中文文件名：
   "上传视频 YYYY-MM-DD NNN"
   - NNN = 该用户当天上传占位名顺序编号（001-999 三位）

4. S2 审校后标题替换：
   Pass 2 输出 display_title_zh → Pipeline POST /job-api/jobs/{job_id}/source-metadata
   Gateway 清洗中文标题后，仅在当前名仍是自动名时替换 display_name：
   - 空 display_name
   - "油管视频/上传视频 YYYY-MM-DD NNN" 占位名
   - 旧版本 yt-dlp 截断 title
   - 旧版本 YouTube video id fallback
   用户通过重命名 API 手动改过的标题不自动覆盖。
```

当前实现中，`YYYY-MM-DD` 由服务端创建任务时的日期提供；后续如前端提供用户时区，可把该日期切到用户本地时区。
...
- 已实现：新建 YouTube 初始名 `油管视频 YYYY-MM-DD NNN`、上传无中文名初始名 `上传视频 YYYY-MM-DD NNN`、上传中文文件名主干命名、同用户冲突后缀 `_xxxx`
- 已实现：S2 Pass 2 输出 `display_title_zh`，Pipeline 在 S2 后回写 Gateway，Gateway 只覆盖自动名，不覆盖用户手动重命名
- 已实现：任务列表以 Gateway DB 的 `display_name` / `expires_at` 为准，避免进入修改页后取消导致列表显示时间被前端/Job API 旧 JSON 覆盖
- 主要代码承接：`src/services/jobs/display_name.py`、`gateway/display_name_orchestrator.py`、`src/services/transcript_reviewer.py`、`src/pipeline/process.py`、`gateway/job_intercept.py`
- 验证：2026-04-25 美国主机真实任务完成后，任务卡标题从占位名更新为 `UFO专家麦卡斯兰将军离奇失踪`
- 部署备份点：`/opt/aivideotrans/deploy_backups/s2_display_title_20260425T124535Z`
...
- Fallback：`display_name` → 非技术来源标题 / 中文上传文件名 → "未命名视频"
- 不再把 Job ID / YouTube video id / 随机 slug 作为用户可见主标题；这些值只保留在详情、日志、下载 fallback 或管理员排障链路中
...
- 自动起名（YouTube / 上传占位名 + 上传中文文件名）+ S2 `display_title_zh` 替换 + 冲突检测 + `_xxxx`
- 重命名 API `PATCH /gateway/jobs/{id}`
- 前端卡片标题切 `title`
- `logs_redactor.py`（此时不接 API，只准备好）
- 单测：宽度 / 起名 / 冲突 / S2 中文标题清洗与覆盖保护 / 脱敏 regex
...
- [x] 任务卡 title 显示 `display_name` 或安全 fallback；2026-04-25 已用真实任务验证 S2 中文标题 `UFO专家麦卡斯兰将军离奇失踪`
