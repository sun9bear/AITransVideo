...
**M1 真实剩余工作**：~~UI-03b 补完~~（✅ #53）→ ~~UI-03e + UI-03f（首页区块）~~（✅ #60 / #61，`/en` 首页全区内容已英文化）→ **UI-03d（SEO 翻旗，下一步，依全部 en 内容=已就位）** ‖ **UI-03c legal 中文源修正(3 blocker)→ en legal（owner 人审门，不自动合）**。**M1 进度**：已合 UI-01/02/03a/03b/03e/03f/04，剩 **03d（SEO，可自动）+ 03c（legal，owner-gated）**。
...
| — | **续接指引（新会话，按序）**：(1) **UI-03d（下一步，SEO 翻旗）**：从最新 origin/main 起分支；per-page `generateMetadata`（self-canonical + 互惠 hreflang 含 en + OG locale，**只在 page 级，绝不进 layout=红线 4**）+ **面包屑 JSON-LD locale 化**（@codex #53 P2）+ EN 排版轨道（globals.css `.en-body`/`--font-en-display` next/font）+ sitemap/robots 含 en + JSON-LD inLanguage locale 驱动 + 收敛 seo 单源。legal 页(terms/privacy/refund/contact)**未译则先不挂 en hreflang**（不阻塞其余页，03c 后补）。**go-live gate**：AnonymousTrialPanel(03g) flag-ON 仍泄漏→先本地化或 /en 关 flag；🔶 en JSON-LD availableLanguage=en-US 待确认英文客服。(2) **UI-03c legal（owner 人审门，不自动合）**：按 [UI-03c-legal-human-review.md](UI-03c-legal-human-review.md) 修 3 blocker(terms 7.2 非续费 / privacy 删百度网盘+通用第三方存储兜底 / refund 9.2 原路退回)；terms 15.2 管辖 + 续费措辞留项目主/律师；改后 cjk-baseline 重生成；en legal + 二轮人审 → **项目主签字后合**。 | — | — | ⏭ 下一步 |
