...
| **UI-03f** | **首页区块英文化（下半）**：Features/SuitedScenarios/TrustBanner/PricingPreview/FinalCta（+ link-button）。⚠️ **primary-cta/plan-card-cta/anonymous-trial-launcher 已由 UI-03b 字典化、03f 只消费不再迁** | ☐ **新增（范围修正）** | Phase 1·T1.2' | M | 是（依 UI-03a/b，与 03e 并行须串行 cjk-baseline） | `uiloc/marketing-en-seo-f` |
...
**M1 真实剩余工作**：~~UI-03b 补完~~（✅ PR #53 已合）→ **UI-03e + UI-03f（首页区块，下一步）** → UI-03c legal 中文源修正(3 blocker)→ en legal → UI-03d(SEO 翻旗，依全部 en 内容)。**M1 进度**：已合 UI-01/02/03a/03b/04，剩 03e/f（首页区块）+ 03c（legal）+ 03d（SEO）。

**新增延后项（UI-03b 发现，待排期）**：
- **AnonymousTrialPanel 本地化（独立单元，暂记 UI-03g）**：`anonymous-trial-panel.tsx`（954 行 / 119 CJK / flag `NEXT_PUBLIC_ENABLE_ANONYMOUS_PREVIEW`-ON 的匿名预览上传·预览·**consent 法务文案**漏斗）。consent 文案有法务权重，应与 03c legal 同等审慎，不混入 leak-fix。**03d 让 /en 上线前 gate**：若该 flag 在 /en 为 ON，panel 仍泄漏中文 → 先本地化 panel 或 /en 关该 flag。
...
| — | **续接指引（新会话，按序）**：(1) **UI-03e + UI-03f（下一步，首页区块英文化）**：各从最新 origin/main 起分支(`git checkout -b ... origin/main` 复用 node_modules)，**串行 cjk-baseline**(03e merge 后 03f 再起)。03e=PainPoints/FeaturedDemos/ProductProof/WorkflowShowcase；03f=Features/SuitedScenarios/TrustBanner/PricingPreview/FinalCta(+link-button)，**CTA primary-cta/plan-card-cta/anonymous-trial-launcher 03b 已字典化、03f 只消费**。每单元逐组件核渲染 CJK(含 aria/alt/placeholder，排注释)、字节一致、cjk 只减、多 lens、PR→@codex→merge。(2) **UI-03c legal 中文源修正**：按 [UI-03c-legal-human-review.md](UI-03c-legal-human-review.md) 修 3 blocker(terms 7.2 非续费 / privacy 删百度网盘+通用第三方存储兜底 / refund 9.2 原路退回)；terms 15.2 管辖 + 续费措辞留项目主/律师；改后 cjk-baseline 重生成；en legal + 二轮人审。(3) **UI-03d** SEO 翻旗(per-page generateMetadata + **面包屑 JSON-LD locale 化**)+ 收敛 seo 单源；**go-live gate**：AnonymousTrialPanel(03g) flag-ON 仍泄漏→先本地化或 /en 关 flag；🔶 en JSON-LD availableLanguage=en-US 待确认英文客服 | — | — | ⏭ 下一步 |
