...
| — | **续接指引（新会话）**：**M1 仅剩 UI-03c legal（owner 人审门，不自动合）** = M1 收口最后一块。按 [UI-03c-legal-human-review.md](UI-03c-legal-human-review.md) 项目主决策修 3 blocker 中文源(terms 7.2 非续费 / privacy 删百度网盘+通用第三方存储兜底措辞 / refund 9.2 原路退回)；terms 15.2 管辖 + 续费措辞留项目主/律师定稿；改后 cjk-baseline 重生成 → en legal 翻译 → **二轮人审 + 项目主签字** → 合并（届时 legal 页一并 generateMetadata + 入 `localizedRoutes` 挂 en hreflang）。**SEO 部署门**（owner，部署时非合并时）：生产暴露 /en 须 owner 部署；go-live 前确认 AnonymousTrialPanel(03g) flag 状态 + en-support 声明。@codex 本会话已用量耗尽（如需续，owner 加 credits）。 | — | — | ⏭ 仅剩 03c |
