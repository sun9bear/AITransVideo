...
| UI-09 | [客户端错误层 + error-code map](UI-09-client-error-layer.md) | ◐ | Phase 3 · T3.4 | M | **设计完成**（branch `uiloc/client-error-layer`；CodeX 建议**前置**于核心工作台铺文案）；实施待做、耦合 UI-BE-01 |
...
| 2026-06-30 | **UI-09 设计细化**（实施待做） | branch `uiloc/client-error-layer` `afa10233`（未开 PR） | 据真实 error 层（client.ts 消息解析 / stringifyErrorDetail 丢弃 error_code / statusFallbackMessage 硬编码中文）细化：本地化在**显示层**做（client.ts 无 translator），ApiError 携 errorCode，新 `localizeApiError(t,error)`：code.→status.→timeout→后端原文兜底，新 `errors` namespace（zh 值照搬现硬编码中文=红线1）。纠正母方案 stale（`LOGIN_ERROR_MESSAGES` 已不存在）。UI-09⊥UI-BE-01 解耦。CodeX 建议 UI-09 **前置**于工作台铺文案 | ◐ 设计完成待实施 |
