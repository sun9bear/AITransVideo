...
**✅ M1 全部完成（2026-06-27）**：UI-01/02/03a/03b/03c/03d-1/03d-2/03e/03f/04 **全部合并 main**。`/en` 营销站内容全英文（首页全区 + hero/pricing/trial + FAQ/对比 + legal bilingual）+ /pricing·/trial 双语 SEO（互惠 hreflang）+ EN 衬线排版。**M1 里程碑边界已达——停等项目主验收，不自主跨入 Phase 2**（UI-05..09 受母方案 §9.2 决策门约束）。⚠️ **SEO 翻旗部署门（owner，部署时非合并时）**：merge≠prod 上线；生产把 /en 暴露给爬虫须 owner 部署，go-live 前 gate = AnonymousTrialPanel flag-ON 时 /en 首页仍泄漏中文（UI-03g）+ en-support 声明 `availableLanguage=en-US`（03a 已合，inert）待确认。
...
| — | **✅ M1 收口（2026-06-27 一会话完成 03e/03f/03d-1/03d-2/03c + 多个 followup/docs）**：**M1 全部单元已合 main，里程碑边界已达**。**下一步 = 停等项目主验收**；不自主跨入 **Phase 2（Wave U-B，UI-05..09）**——受母方案 §9.2 决策门约束。**M1 收尾遗留（owner/后续）**：① **UI-03g** AnonymousTrialPanel 本地化（落地须把 home `/` 加回 `localizedRoutes` + 还原 home hreflang-check/zh-snapshot 断言）；② **SEO 部署门**：生产暴露 /en 给爬虫须 owner 部署（Via-154），go-live 前确认 AnonymousTrialPanel flag 状态 + en-support 声明 `availableLanguage=en-US`；③ legal 若想改 locale-switch（zh-only /terms、en-only /en/terms）可用现成 en 文本 refactor 成 dict（可选）。 | — | — | ✅ M1 done |
