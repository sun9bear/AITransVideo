...
| `gateway/alembic/versions/028_pan_backup.py` | migration:加 3 表 + status vocab/触点扩展 + 索引 |
| **`src/services/jobs/models.py`** | `Job API JobStore` 状态验证 set 加 3 个新值。当前 line 229 `SUPPORTED_JOB_STATUSES`(verified) 会拒未知状态 ─ 不改则 set_archive_status 写 JSON store 立即抛 |
| **`src/services/web_ui/cleanup.py:53`** | `_CLEANUP_PROTECTED_STATUSES` 加 `archiving` / `restoring`。`archived` 不进(那是 cleanup 的预期对象,虽然实际由 backup_executor 主动删,但 cleanup 至少不会 race-delete) |
| **`frontend-next/src/types/jobs.ts` + `api.ts`** | `JobStatus` TypeScript union 加 3 个值,否则 status 过滤器 + UI badge 编译错 |
| **`frontend-next/src/lib/i18n/` 或 status label 表** | 中文 label:`archiving='归档中'` / `archived='已归档'` / `restoring='恢复中'`。badge 颜色:archiving=橙 / archived=灰 / restoring=蓝 |
| **API contract tests** | `tests/test_status_vocab_in_sync.py`(新建):assert Gateway/Job API status vocab(`SUPPORTED_JOB_STATUSES`) ∩ TS union 三处一致 |
...
| `gateway/config.py` | `GatewaySettings` 加 §5.1 所列 pan/baidu 配置字段 |
...
| `gateway/storage/event_log.py` | `_DOWNLOAD_EVENT_TYPES` **不改名**(line 58-59 注释明确"kept for git-blame continuity")— 只是 frozenset 集合加 8 个 pan.* |
...
CREATE TABLE backup_records (
    ...
    -- 关键:绑定该 backup 是任务的"第几代"(edit_generation 来自 JobRecord.edit_generation)
    -- 任务被 restore → edit → re-archive 后 edit_generation 涨,新 backup_record 行
    -- 不被旧 'restored' 行卡住 scanner
    job_edit_generation INT NOT NULL DEFAULT 0,
    provider           VARCHAR(32) NOT NULL,
    remote_path        TEXT NOT NULL,
    size_bytes         BIGINT NOT NULL,
    sha256             VARCHAR(64) NOT NULL,
    md5                VARCHAR(32) NOT NULL,   -- 百度 Pan API 用,用于 commit-point 校验
    manifest_json      JSONB NOT NULL,
    status             VARCHAR(32) NOT NULL,   -- uploading | uploaded | failed | restoring | restored | deleted
    heartbeat_at       TIMESTAMPTZ,            -- 长时运行心跳(见 §10 stale_reaper)
    created_at         TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    completed_at       TIMESTAMPTZ,
    error_message      TEXT
);

-- 一个 (user, job, provider, edit_generation) 任何时刻最多一个**进行中**的 backup/restore
-- 注意:edit_generation 进 UNIQUE,允许同任务跨 generation 多次 archive
CREATE UNIQUE INDEX uniq_backup_in_flight
    ON backup_records (user_id, job_id, provider, job_edit_generation)
    WHERE status IN ('uploading', 'restoring');

CREATE INDEX idx_backup_user_status ON backup_records (user_id, status);
-- 给 archive_scanner 的 NOT EXISTS 子查询用(必须含 edit_generation 维度)
CREATE INDEX idx_backup_user_job_gen ON backup_records (user_id, job_id, job_edit_generation);
-- 给 stale_reaper 用(扫 heartbeat_at 过期的 in-flight 行)
CREATE INDEX idx_backup_heartbeat ON backup_records (heartbeat_at) WHERE status IN ('uploading', 'restoring');
...
```bash
# 双 Feature flag (CodeX P1-6 fix: 拆开手动 vs 自动)
AVT_ENABLE_PAN_BACKUP=false           # 主开关:OAuth + manual backup/restore endpoints + UI
AVT_PAN_AUTO_ARCHIVE_ENABLED=false    # 独立子开关:30d 自动归档 cron
                                      # 先手动灰度 1 周 → 再开 auto,避免 OAuth 接入瞬间触发大规模外部 API
...
# 自动归档调度(仅 AVT_PAN_AUTO_ARCHIVE_ENABLED=true 时生效)
AVT_PAN_AUTO_ARCHIVE_DAYS=30          # 阈值(天)
AVT_PAN_AUTO_ARCHIVE_HOUR_BJT=3       # cron 触发小时
AVT_PAN_AUTO_ARCHIVE_MAX_PER_RUN=5    # 每次 cron 最多 enqueue 任务数(原 100 太激进,跨境上传带宽<5MB/s 容易堆)
AVT_PAN_AUTO_ARCHIVE_DRY_RUN=true     # **新**:首次开启时强烈建议 true → 只 log candidates 不 enqueue
                                      # 灰度跑 1-2 晚确认 candidate 选择正确后改 false

AVT_PAN_ORPHAN_CLEANUP_WEEKDAY=5  # 周六=5

# 上传配置
AVT_PAN_UPLOAD_CHUNK_BYTES=4194304   # 4MB
AVT_PAN_TASK_STALE_HOURS=4           # heartbeat 心跳过期阈值(default 4h)。
                                     # heartbeat_at 在 backup 期间每 60s 刷新,4h 没刷 = executor 挂了。
                                     # 历史 100GB 跨境单任务也通过 heartbeat 持续报"活",不会被误杀。
                                     # 与 §16.4 单任务最长 2-3h 的现实期望一致。
```
...
| `DELETE` | `/admin/pan/backups/{id}`                 | **软删** PG 行(status='deleted')+ 删远端 tar.gz。**Guard**:若关联 job.status='archived' 且当前 generation 没有其它 'uploaded' backup_record,**拒绝**(412);admin 必须先 restore 或保留唯一可恢复副本 |

### `DELETE /admin/pan/backups/{id}` 保护逻辑(伪代码)

```python
async def delete_backup(backup_id: UUID):
    br = await fetch_backup(backup_id)
    if br.status == 'deleted':
        return 204  # idempotent
    
    # 拒绝条件:被保护任务的唯一可恢复副本
    if br.status == 'uploaded':
        job = await fetch_job(br.user_id, br.job_id)
        if job and job.status == 'archived' and job.edit_generation == br.job_edit_generation:
            # 这是 archived 任务的当前 generation 副本,检查同 generation 是否还有其它 'uploaded'
            sibling_count = await db.scalar("""
                SELECT count(*) FROM backup_records
                 WHERE user_id=:u AND job_id=:j AND job_edit_generation=:g 
                   AND status='uploaded' AND id != :id
            """, u=br.user_id, j=br.job_id, g=br.job_edit_generation, id=backup_id)
            if sibling_count == 0:
                raise HTTPException(412, 
                    "Refuse: 这是 archived 任务的唯一可恢复副本。先 restore 后再 delete,"
                    "或者通过另一次 backup 留新副本。")
    
    # 通过保护 → 软删 PG + 物理删 pan
    await baidu_pan_client.delete(br.remote_path)  # 失败仅 log,继续 PG soft delete
    await db.execute("UPDATE backup_records SET status='deleted' WHERE id=:id", id=backup_id)
    return 204
```

**审计价值**: 软删(`status='deleted'`)而非物理 DELETE,保留行用于审计/取证。orphan_cleanup 在远端扫到匹配 `status='deleted'` 的行视为已批准的清理,不重复操作。
...
Executor (gateway/background_task_executors.py:execute_pan_backup)
   ...
   a. **Session-level** advisory lock:`SELECT pg_advisory_lock(hash(user_id, job_id))`
      ❌ 不用 `pg_try_advisory_xact_lock`:那是事务级,锁会持续整个事务。
         backup 跨 tar / sha256 / 跨境上传可能 1-3h,大事务会拖死 DB。
      ✅ Session-level:锁绑 PG 连接,executor 跑完(succ/fail/crash)断连释放,
         所有 PG 写操作走独立短事务。
      ⚠️ Connection-pool 需保证 executor 整个生命周期持有同一个 connection
         (用 `async with engine.connect() as conn:` 显式 hold)。
   ...
   c. INSERT backup_records (status='uploading', job_edit_generation=job.edit_generation, heartbeat_at=now())
      并启动后台 task:每 60s `UPDATE backup_records SET heartbeat_at=now() WHERE id=...`
      → stale_reaper 用 heartbeat_at 判活,不再依赖 jobs.updated_at(那个不在 backup 中刷)
   ...
   f. 算 tar.gz 总 sha256 + 总 md5(百度 Pan API 用 md5)
   g. Baidu Pan 上传
      ├ decrypt access_token,expired 则 refresh
      ├ precreate API: 声明 path + size + **每片 md5 列表 + 总文件 md5**
      ├ 4MB 分片 PUT,每片记 md5
      └ finish API: 合并 chunks,response 返**服务端计算的 md5 + fs_id + size**
   h. **三道 checksum 闸门**(只有全过才算 commit-point ready)
      ├ h1. Size match: HEAD remote = local size ─── 防截断
      ├ h2. MD5 match: finish API 返的 server_md5 == 本地 md5 ─── 防内容损坏
      └ h3. **Read-back probe**: HTTP Range GET 末尾 64 KB,本地算 sha256 比对同区段。
            理由:md5 防 transparent 内容篡改,read-back 防"finish API 报告 md5 但实际存 0 字节 / 别的文件"
            (云存储偶发的元数据-内容不一致 bug)。开销可忽略(64 KB 几百 ms)。
   i. UPDATE backup_records SET status='uploaded', sha256, md5, size_bytes, completed_at  ◄── COMMIT POINT
      此刻 tar.gz 在 pan + size + md5 + 末尾 64KB sha256 三重验证,**用户数据已安全**,后续任一步失败 archived 不回滚
      ⚠️ 任一闸门(h1/h2/h3)失败 → rollback succeeded + backup_records=failed,**不删 local/R2**
   ...
   l. set_archive_status(job_id, 'archived'),停 heartbeat 后台 task
   m. 释放 session advisory lock:`SELECT pg_advisory_unlock(hash(user_id, job_id))`(或 connection close 自动释放)
...
WHERE u.role = 'admin'
  ...
  -- 关键:只排除**当前 generation** 的 in-flight / 已 archive 行
  -- 历史 generation 的 restored 行不卡(允许 restore→edit→re-archive 路径)
  AND NOT EXISTS (
...
LIMIT :max_per_run;                                -- AVT_PAN_AUTO_ARCHIVE_MAX_PER_RUN,默认 5
```
...
**`ORDER BY` 索引匹配**: 见 §4.3 `idx_backup_user_status` + `idx_backup_user_job_gen`(NOT EXISTS 用)+ jobs 表 `idx_jobs_user_status_updated`(若不存在,migration 028 加一个 partial: `WHERE status='succeeded'`)。

### orphan_cleanup 算法(周六同时跑 3 道清理)

```
Pass A — Baidu Pan 远端孤儿
  ...
  3. pan_orphans = remote_files - db_paths
  4. For each orphan:
       ├ safety guard: path 必须 prefix /apps/AIVideoTrans/backups/
       ├ baidu_pan_client.delete(path)
       └ emit_event('pan.orphan.cleaned', path)

Pass B — R2 残留(archived job 的 R2 key 没删干净)
  1. SELECT job_id, user_id, r2_artifacts FROM jobs WHERE status='archived' AND r2_artifacts IS NOT NULL
  2. For each row:
       ├ for each r2_key in r2_artifacts:
       │   ├ r2_client.delete_object(Bucket, Key=r2_key) — 失败 log,continue
       │   └ 成功后从 jobs.r2_artifacts JSONB 移除该 entry
       └ emit_event('pan.r2_residue.cleaned', job_id=..., keys=[...])
  3. 此 pass 跟 backup_executor 的步骤 k 是同一段清理逻辑,只是兜底重试 — backup_executor 第一次清完一般这里没活干

Pass C — pan_oauth_states 过期
  1. DELETE FROM pan_oauth_states WHERE expires_at < now();
```

3 个 pass 串行跑,任一失败不影响其它。一周一次的成本可接受。
...
async def reap_stale_pan_jobs():
    """heartbeat 超 AVT_PAN_TASK_STALE_HOURS(default 4h)→ commit-aware 处理。
    
    判活基准从 `jobs.updated_at` 改为 `backup_records.heartbeat_at`:
    - jobs.updated_at 在 backup 期间不刷新(只 archiving→archived 时刷)
    - backup_records.heartbeat_at 每 60s 刷,活 executor 必然新鲜
    
    并发安全:取 session-level 同款 advisory lock,如果原 executor 还活着持锁,
    我们 pg_try_advisory_lock 会立即返 false → 跳过这个任务下一轮再看。
    
    ⚠️ pg_try_advisory_lock(non-xact 版)是 session 级。reaper 在跑完每一行
    后必须显式 pg_advisory_unlock,否则锁随 reaper session 持有不释放。
    """
    
    # --- 找心跳过期的 in-flight backup ---
    stuck_in_flight = await db.execute("""
        SELECT br.id AS backup_id, br.user_id, br.job_id, br.job_edit_generation,
               br.status AS backup_status,
               j.status AS job_status
          FROM backup_records br
          JOIN jobs j ON j.user_id=br.user_id AND j.job_id=br.job_id
         WHERE br.status IN ('uploading', 'restoring')
           AND (br.heartbeat_at IS NULL 
                OR br.heartbeat_at < now() - interval '4 hours')   -- AVT_PAN_TASK_STALE_HOURS
    """)
    
    for row in stuck_in_flight:
        # session-level 非事务 try-lock
        got = await db.scalar("SELECT pg_try_advisory_lock(:k)", k=hash_user_job(row.user_id, row.job_id))
        if not got:
            continue  # 原 executor 还活着(虽然 heartbeat 老了,但锁还在 → 锁住 PG 连接,稍后再看)
        
        try:
            if row.backup_status == 'uploading':
                if row.job_status == 'archiving':
                    # 未过 commit point(若已 commit,backup_status 已是 'uploaded')。
                    # 安全 rollback 到 succeeded,数据没出 local。
                    await set_archive_status(row.user_id, row.job_id, 'succeeded')
                    await db.execute(
                        "UPDATE backup_records SET status='failed', error_message='reaped after heartbeat 4h' WHERE id=:id",
                        id=row.backup_id,
                    )
                    emit_event('pan.backup.failed', job_id=row.job_id, payload={'reason': 'stale_reap', 'phase': 'pre_commit'})
                # else: 罕见,job_status 已不是 archiving 但 backup_records 还 uploading,只清 backup_records
            
            elif row.backup_status == 'restoring':
                # Restore 失败 → 必须回 archived(tar 还在 pan)
                await safe_rmtree_project_dir(row.job_id)
                await set_archive_status(row.user_id, row.job_id, 'archived')
                await db.execute(
                    "UPDATE backup_records SET status='uploaded', error_message='restore reaped after heartbeat 4h' WHERE id=:id",
                    id=row.backup_id,
                )
                emit_event('pan.restore.failed', job_id=row.job_id, payload={'reason': 'stale_reap'})
        finally:
            await db.execute("SELECT pg_advisory_unlock(:k)", k=hash_user_job(row.user_id, row.job_id))
    
    # --- 单独扫:job=archiving + backup_record.status='uploaded' 但 status_mutator 没写完 ---
    # 这是 §11 row "set_archive_status('archived') 写崩" 场景。executor 已 commit point,
    # 但状态字段没更到 archived(crash / 网络),此时无 in-flight backup_records 来锁。
    stuck_post_commit = await db.execute("""
        SELECT j.user_id, j.job_id, br.id AS backup_id
          FROM jobs j
          JOIN backup_records br 
            ON br.user_id=j.user_id AND br.job_id=j.job_id 
           AND br.job_edit_generation=j.edit_generation 
           AND br.status='uploaded'
         WHERE j.status='archiving'
           AND br.completed_at < now() - interval '4 hours'   -- 已 commit 但没 forward
    """)
    for row in stuck_post_commit:
        got = await db.scalar("SELECT pg_try_advisory_lock(:k)", k=hash_user_job(row.user_id, row.job_id))
        if not got:
            continue
        try:
            # forward-resolve:数据在 pan + sha256/md5/range probe 三验过,直接推 archived
            await set_archive_status(row.user_id, row.job_id, 'archived')
            await enqueue_background_task('pan_residue_cleanup', {
                'job_id': row.job_id, 'user_id': row.user_id, 'backup_id': str(row.backup_id),
            })
...
(`restoring` 卡死走上面 `stuck_in_flight` 统一循环的 `backup_status='restoring'` 分支,不再单独写。)

**新增 background task `pan_residue_cleanup`**:接收 `{job_id, user_id, backup_id}`,**先取 `pg_try_advisory_lock`**(session 级,避免和挂尸 executor 撞),拿不到就 reschedule 10min 后重试;拿到后重试 `shutil.rmtree(project_dir)` + 逐个 `r2_client.delete_object` + UPDATE jobs.r2_artifacts。完成后 `pg_advisory_unlock`。失败仍 archived 状态(数据安全),周末 orphan_cleanup 兜底。
...
| heartbeat 4h 过期 archiving | stale_reaper 扫到 (default `AVT_PAN_TASK_STALE_HOURS=4`) | commit-aware:已 uploaded → forward-resolve archived;uploading → rollback succeeded。reaper 取 session-level `pg_try_advisory_lock`,失败 skip 下轮 | notifications_service 通知 admin |
| heartbeat 4h 过期 restoring | 同上 | rollback archived,清 partial project_dir,backup_records 还原 uploaded。同样 session lock 保护 | 同上 |
...
pan.restore.failed
pan.orphan.cleaned      (内部事件,不 surface 到 admin UI)— pan 远端孤儿
pan.r2_residue.cleaned  (内部事件)— R2 残留兜底清理
```

### 12.2 同步点(契约测试守)

- `src/services/jobs/events.py:SUPPORTED_EVENT_TYPES` 加 8 个
- `gateway/storage/event_log.py:_DOWNLOAD_EVENT_TYPES` **保留命名**(line 58-59 注释"kept for git-blame continuity"),只是 frozenset 加 8 个 pan.*
...
| `tests/test_pan_stale_reap.py` | archiving heartbeat 4h 过期 → reap(commit-aware:uploaded → forward-resolve archived;uploading → rollback succeeded);restoring heartbeat 4h 过期 → rollback archived;**所有分支 mock `pg_try_advisory_lock` 测拿不到锁时 skip** |
...
| 模块 | 工日 |
|------|------|
| Schema + alembic 028 + status vocab/触点扩展 + 3 表 model + edit_generation/heartbeat 列 | 0.8 |
| Status 三处同步(gateway PG model + Job API JSON store + frontend TS union) + i18n + contract test | 0.5 |
| OAuth Web Flow + token refresh + Fernet + recipe registration | 2.0 |
| Baidu Pan API client(OAuth + refresh + 分片上传 + **服务端 md5 校验** + 下载 + list + delete + quota) | 2.2 |
| backup_executor + restore_executor + manifest + status_mutator + **heartbeat 后台 task** + **read-back probe** | 2.5 |
| archive_scanner(dry-run + per-run limit)+ orphan_cleanup(3 pass)+ **stale_reaper(commit-aware,新 infra)**+ residue_cleanup | 1.2 |
| Admin UI(dashboard + backups page + 任务列表按钮 + 状态过滤器 + Restore 流程 + DELETE 保护提示) | 2.5 |
| Events + observability + r2_observability 扩展 + 契约测试更新(含状态 + event vocab + DELETE guard) | 0.8 |
| Tests(8 unit + 2 contract + smoke + e2e checksum 校验场景) | 1.8 |
| **合计** | **~14.3 工日** |

> 比 round 2 估的 12 工日再 +2.3 天。原因:CodeX P0/P1 引入的额外工作(checksum 三道闸门 / edit_generation 维度 / DELETE 保护 / heartbeat / status 三处同步 / 双 flag + dry-run)都是 correctness/safety 必需,不是 nice-to-have。
...
| O2 | Orphan cleanup 集成 vs 独立脚本 | 独立脚本,周六,3 pass(pan + R2 残留 + oauth_states)(D10) |
| O3 | backup 是否需要并发 | ❌ 不需要(D11),全局 sem 1 |
| O4 | OAuth 接入方式 | 完整 Web Flow(D7) |
| O5 | 通知通道 | `notifications_service.dispatch_event` 站内(D13) |
| O6 | 工作量预估 | **~14.3 工日**(§15,含 CodeX P0/P1 修订) |
| O7 | Data-safety:commit point 前的校验 | 三道闸门:size + 服务端 md5 + 末尾 64KB read-back probe(§7 P0-1) |
| O8 | restored 历史行 → 卡 scanner | `backup_records` 加 `job_edit_generation`,scanner WHERE 含此维度(§4.3 P0-2) |
| O9 | DELETE 端点删唯一副本风险 | 412 拒删 + 软删 + 审计行保留(§6 P0-3) |
| O10 | Advisory lock 跨小时上传 | session-level lock + heartbeat 每 60s + reaper 判 heartbeat_at(§7 + §10 P1-4) |
| O11 | 自动归档独立 flag | `AVT_PAN_AUTO_ARCHIVE_ENABLED` 独立 + dry-run 默认 + per-run 5(§5.1 P1-6) |
