...
| 项目 | 计划口径 | 当前状态 | 结论 |
|---|---|---|---|
| Phase 0 探针 | 三类探针都要完成并写入基线 | 探针 ① + ② 三网全数据已进 §15.2 / §15.3（2026-04-23，武汉样本，探针 ② 最差 2 MB/s > 下限 2×，D39 判据通过）；探针 ③ 未跑 | 探针 ①②完成（MVP 放行），③ 待跑 |
...
- 方案的 `§15` 已填入：
  ...
  - **探针 ② 三网完整数据**（2026-04-23，武汉本地抽样，5 次/运营商 presigned GET）：
    - 电信平均 4.08 MB/s（min 3.00, max 6.30, 5/5 成功）
    - 联通平均 4.45 MB/s（min 4.08, max 5.09, 5/5 成功）
    - 移动平均 2.00 MB/s（min 1.33, max 3.02, 5/5 成功）
    - **D39 判据通过**：三网平均 ≥ 1 MB/s ✅ 成功率 ≥ 90% ✅ → MVP 走 v4 主路径
    - 观察项：移动在 R2 直连路径最慢（2.00 MB/s），与探针 ①（17.10 MB/s via CF tunnel）
      形成 8.5× 倒挂；MVP 后如移动体感差，可考虑把 R2 挂 CF 自定义域名（轻量版 Phase 2b）
  - `17ce.com` 覆盖测试补充说明

未落实或证据不足的部分：

- 探针 ③ 尚未跑三网数据（需要项目方发受限 R2 token）。
- 地理覆盖仅武汉本地一点，沿海（上海/广深）与北方（北京/成都）抽样待补。
- 生产环境 `.env` 中没有任何 `R2_*` 或 `AVT_STORAGE_BACKEND` 标记，说明 Phase 0 探针数据还没转化成后续 R2 上线配置。

判断：

- **Phase 0 探针 ① + ② 已充分落地，MVP v4 主路径绿灯放行**。
- 探针 ③ 是 Phase 3（上传路径）放行门槛，Phase 2（下载路径）不依赖 ③。
- 推进节奏：现在可以开 Phase 2 最小 R2 下载闭环（代码侧 302 → R2 presigned），
  探针 ③ 和 Phase 3 上传路径可以并行或稍后做。
