...
**日期**：2026-05-27  
**状态**：`IN_PROGRESS` —— Phase 0a / 0b / 1 / 2 / 3 ✅ **后端均已部署美国主机并运行态验证**（omni→v2.5、成本目录、真实 usage 采集、TTS V2.5 主动升级 + 限免标记）；前端 admin 文案改动已 commit 未部署（待下次前端发布）；Phase 4 / 5（试点 / benchmark / 默认决策）待做  
...
- ✅ **已部署美国主机（2026-05-29）**：在飞 job guard=0 后，`git archive HEAD` 打包 4 文件（llm_registry / usage_meter / translator / transcript_reviewer）→ 上传 → 解包 bind-mount → `docker restart aivideotrans-app`；运行态验证三模块导入无错、`normalize_openai_usage` 返回正确拆分、PR 0b 仍在、app healthy、公网 200；4 文件留 `.bak-20260529-pr2`。
...
- ✅ **smoke 通过**（美国主机 `docker exec`，免费）：`mimo-v2.5-tts` + voice `mimo_default` 对中文 / 中英混合 / 短句均返回有效 WAV（38–146KB）；`mimo-v2-tts` 回退仍可用；TTS 返回 usage（token-based）。**真实格式 = `modalities:["audio"]` + `audio:{voice,format}`**（spike 朴素 chat payload 400 的原因）。
- ✅ `DEFAULT_MIMO_MODEL` 升 `mimo-v2.5-tts`，`os.environ["MIMO_TTS_MODEL"]` 可运行时回退；docstring 同步。
- ✅ cost catalog 加 `mimo:mimo-tts` / `mimo-v2.5-tts` / `mimo-v2-tts` 三个 **promotional** 条目（`billed_chars=0` 不伪造）；`apply_costs` 新增 `rate_status="promotional"` 分支（仅 promotional provider，paid provider 仍 `configured`，有守卫测试）；catalog version → `2026-05-29-mimo-v2.5-llm-tts`。
- ✅ 前端 `admin/settings` 文案 → "MiMo-V2.5-TTS（小米）" + 限免随时转收费提示。
- ⏳ **TTS token usage 未采集**：TTS 现免费 + 成本模型基于 `billed_chars`（TTSRow 无 token 字段），采集即死数据；待 TTS 转收费且按 token 计费再做。保持 `billed_chars=0` 不伪造。
- ✅ 测试 `tests/test_mimo_tts_v25.py`（默认 v2.5 / env 回退 / promotional 状态 / paid 不误标）+ cost/PR2/migration 28 条全绿。
- ⚠️ 旁注：本地跑 `test_tts_generator.py` 有 12 个 **cosyvoice** 测试失败（网络重试 >150s 超时，本地够不到 voice_catalog DB / dashscope）——环境问题，与本次 mimo 改动无关（未碰 cosyvoice / tts_generator）。
- ✅ **后端已部署美国主机（2026-05-29）**：在飞 guard=0 后，`mimo_tts_provider.py`（app bind-mount）→ restart app，运行态 `DEFAULT_MIMO_MODEL=mimo-v2.5-tts` ✅；`cost_management.py`（gateway 镜像）→ build + `--no-deps --force-recreate`，运行态 version=`2026-05-29-mimo-v2.5-llm-tts` + 5 个 mimo catalog 条目 ✅；5 容器 healthy、公网 200；两文件留 `.bak-20260529-pr3`。
- ⏳ **前端文案未部署**：`admin/settings` label 改动已 commit（`9b56e70`），但需 Next.js 镜像重建（5–15min，纯 admin 装饰性），未部署——随下次前端发布走，或按需单独部署。功能不受影响（选 "mimo" 后端已用 v2.5-tts）。
...
- **2026-05-29 commit + PR 2 完成 + 部署**：
  - 已 commit：`ed61a12`（Phase 0a/0b/1）→ `6814b22`（PR 2 pt1 translator）→ `2b4e65b`（PR 2 pt2 reviewer + DRY normalizer）。drift 消除，git HEAD 与生产一致。
  - PR 2 完整实现：translator（translate/rewrite/probe/content_compliance）+ transcript_reviewer（pass1/pass3/legacy review）真实 usage 采集；`normalize_openai_usage` 抽到 llm_registry 单一真源；按 spike 实测拆 `input=prompt−cached−audio` 避免双重计费；全程 best-effort 不破坏主流程；Gemini SDK path 未碰。
  - 已部署美国主机并运行态验证；4 文件 `.bak-20260529-pr2` 回滚点。
  - 旁注：`test_smart_cost_summary_writer.py` 工作树有非本次的改动（可能是 quota-brake 排查任务），未纳入任何本次 commit。
- **2026-05-29 Phase 3 完成 + 后端部署**：commit `9b56e70`。smoke 确认 `mimo-v2.5-tts` 真实格式（`modalities`+`audio`）+ voice `mimo_default` 可用、v2-tts 回退可用、TTS 返回 token usage。决策：(1) 切默认 `mimo-v2.5-tts` + `MIMO_TTS_MODEL` env 运行时回退；(2) TTS 限免用 cost catalog `promotional` 状态表达（不伪造 billed_chars，不当永久 0 成本）；(3) **不采集 TTS token usage**（免费 + char-based 成本模型 = 死数据，YAGNI）；(4) Smart TTS 仍 MiniMax 不动。后端（app+gateway）已部署验证；前端 admin 文案 commit 未部署（Next.js 重建成本高、纯装饰）。cosyvoice 本地测试失败为环境问题，与本次无关。
