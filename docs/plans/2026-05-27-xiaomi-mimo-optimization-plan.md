...
- ⏳ **PR 2 第二部分待做**：transcript_reviewer 的 MiMo 音频审核路径（`_call_mimo_omni_raw` 返回 usage + reviewer `_record_llm_usage` 透传）。非默认路径（Pass1/2/3 默认 gemini_pro，仅 admin 选 MiMo 才走），独立增量。
- ⏳ 未部署美国主机（PR 2 是计量精度改进，无 deadline，建议与第二部分一起部署）。
