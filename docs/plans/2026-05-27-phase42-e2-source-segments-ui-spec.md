...
**作者：** Claude Code
**版本：** v2.1（2026-05-27 Codex 二轮复审：常量符号 + picker 回传契约二处订正）
**日期：** 2026-05-27
...
2. **客户端总时长阈值固定 3.0 – 60.0 秒**（与后端 `gateway/cosyvoice_clone/sample_validator.py` 的 `MIN_DURATION_MS = 3_000` / `MAX_DURATION_MS = 60_000` 一致——注意单位是毫秒）。
   - 已选段总时长 `< 3.0s` → 红色提示"还需 X.Xs"，**禁用** 提交按钮
   - 已选段总时长 `> 60.0s` → 红色提示"超出 X.Xs"，**禁用** 提交按钮
   - 后端仍是最终权威；前端把决策提前避免发出去才被拒。
   - **守卫 cross-check**：测试扫前端有 `3` / `3.0` / `60` / `60.0` 字面量，扫后端 `sample_validator.py` 含 `MIN_DURATION_MS = 3_000` 和 `MAX_DURATION_MS = 60_000` 字面量；两侧任一变了会同步红。注意单位不同——前端 seconds，后端 ms，守卫做 1000× 换算后比对。
