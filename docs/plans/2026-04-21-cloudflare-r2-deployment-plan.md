...
- aitrans.video 经 CF 边缘后电信 / 联通（武汉）两路径均恢复可达
- 下载速度两网均远高于 § 11.3 MVP 放行下限（1 MB/s）：
  - 电信 9.08 MB/s ≈ 73 Mbps（9× 裕度）
  - 联通 5.11 MB/s ≈ 41 Mbps（5× 裕度）
  - 两网下载性能都可以支撑中等规模 final_video 直接拉流
- API TTFB / P50：
  - 电信 TTFB 1166ms / P50 951ms
  - 联通 TTFB 854ms  / P50 844ms（明显更快；武汉联通跨境到 CF 边缘延迟稍低）
  - 两网皆 ~1s 级，对首屏"慢但可用"的 SaaS 可接受，不进 MVP 门槛；
