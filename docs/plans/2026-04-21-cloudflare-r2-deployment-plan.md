...
| 中国联通 | 武汉 | 854 | 844 | 5.11 | 否（直连可达 HTTP 200） |
| 中国移动 | 武汉 | 882 | 904 | **17.10** | 否（直连可达 HTTP 200） |
...
**对照结论（三网齐，武汉本地抽样，2026-04-23）：**
- GFW 对 US IP 直连端口 443 维持 RST 封锁（历史数据）
- aitrans.video 经 CF 边缘 + Phase 1 Tunnel 后，**三网路径均恢复可达**（HTTP 200）
- **下载速度全部远高于 § 11.3 MVP 放行下限（1 MB/s）**：
  - 移动 **17.10 MB/s** ≈ 137 Mbps（**17× 裕度**）——三网最快
  - 电信 9.08 MB/s ≈ 73 Mbps（9× 裕度）
  - 联通 5.11 MB/s ≈ 41 Mbps（5× 裕度）
  - 三网都能支撑中等规模 final_video 直接拉流
- API TTFB / P50：
  - 联通 TTFB 854ms  / P50 844ms（API 最稳；P50 抖动最小）
  - 移动 TTFB 882ms  / P50 904ms（三网最稳定，10 次样本 IQR 仅 ~60ms）
  - 电信 TTFB 1166ms / P50 951ms（偏慢但仍可接受）
  - 三网皆 ~1s 级，对首屏"慢但可用"的 SaaS 可接受，不进 MVP 门槛；
    未来如用户体感反馈慢再评估 CF Argo / Smart Routing
- **放行意义**：三网下载最差 5 MB/s（联通），相对 §11.3 判据的 1 MB/s 下限
  具备 5× 以上冗余。探针 ① 这一维度**已充分支持 MVP 走 v4 主路径**，
  探针 ② 只要能复现相当水平（或不劣化到 1 MB/s 以下），则可直接拍板；
  探针 ② 结果即便不如 ①，也只会影响是否叠加 Phase 2b 备胎，不会阻断 MVP。

**地理覆盖注意**：三网样本全为武汉本地，沿海（上海 / 广州 / 深圳）
可能跨境路径完全不同。本数据仅作为武汉特定链路的起点；GA 前至少再
补一轮沿海 + 北方（北京 / 成都）抽样以免以武汉代全国。
...
**当前基线（2026-04-23，R2 presigned GET，6h TTL，三网武汉）：**

| 运营商 | 平均速度 (MB/s) | min (MB/s) | max (MB/s) | 成功次数 / 5 | RST 次数 | 备注 |
|--------|---------------|-----------|-----------|-------------|---------|------|
| 中国电信 | 4.08 | 3.00 | 6.30 | 5/5 | 0 | 正常 |
| 中国联通 | 4.45 | 4.08 | 5.09 | 5/5 | 0 | 正常（速度最稳） |
| 中国移动 | 2.00 | 1.33 | 3.02 | 5/5 | 0 | ⚠️ 达标但裕度小，min 1.33 距 1 MB/s 下限仅 33% |

**历史对照（2026-04-22，R2 presigned GET，单运营商，300s 截断）：**

| 运营商 | 平均速度 (MB/s) | min (MB/s) | max (MB/s) | 成功次数 / 5 | RST 次数 | 备注 |
|--------|---------------|-----------|-----------|-------------|---------|------|
| 中国电信 | 0.34 | 0.15 | 0.78 | 5/5 | 0 | ⚠️ 可达但慢；第1次128s完成(0.78)，第2-5次均超300s限速截断 |

> 历史对照是 Phase 0 探针端点未上线前的一次人工抽测，当时 R2 bucket 的样本
> 配置可能未完全到位、也不是本次的同一 presigned URL；新数据由完整三网同学在
> `probe2_r2_download.sh` 脚本下跑出来，可信度更高。
...
**判据（D39 触发）**：
- [x] 三网平均 ≥ 1 MB/s 且成功率 ≥ 90% → **R2 原生域名可用**，MVP 按 v4 推进 ✅ 2026-04-23 通过
- [ ] 任一运营商 < 1 MB/s 或成功率 < 90% → **启用 Phase 2b**（public bucket + Worker HMAC）

**结论（2026-04-23，三网齐活）：**

✅ **Phase 2 主路径放行**。三网平均 2.00-4.45 MB/s，成功率 100%，无 RST。

⚠️ **观察项（不阻断，但需 MVP 后监控）：**

1. **探针 ① vs 探针 ② 的运营商排序倒挂**：
   | 运营商 | ① US-tunnel DL (MB/s) | ② R2-direct DL (MB/s) | ①/② 倍数 |
   |--------|----------------------|----------------------|----------|
   | 电信   | 9.08                 | 4.08                 | 2.2×     |
   | 联通   | 5.11                 | 4.45                 | 1.1×     |
   | 移动   | **17.10**            | **2.00**             | **8.5×** |

   同一武汉移动链路，经 CF 边缘 + Tunnel 回 US 比直连 R2 原生域名快 8 倍。
   意味着移动用户走"aitrans.video → CF 边缘 → 回源 US 读 R2"比"aitrans.video
   → 302 → r2.cloudflarestorage.com"顺滑得多。

2. **移动直连 R2 在 min 端仅 1.33 MB/s**，距 1 MB/s 下限仅 33% 裕度。
   一场 500MB final_video 下载最坏情况约 6 分 15 秒（实测极值），
   用户感知为"要等很久"。这不是放行阻塞项，但是体感红线。

3. **设计建议（Phase 2 之后）**：
   - 默认继续走 v4 主路径（R2 presigned 302 直连）——代码最简单、成本最低
   - MVP 上线后观测移动用户的 final_video 下载完成率 / 中断率
   - 如移动投诉集中，可把 R2 挂 CF 自定义域名（如 `files.aitrans.video →
     r2 bucket`），让 R2 流量也经 CF 边缘返回——相当于借 CF Smart Routing
     把移动速度拉回 17 MB/s 级。这是 Phase 2b 的轻量版本（不上 Worker HMAC，
     只加 DNS + bucket public access-via-custom-domain）

4. **地理覆盖**：三网全为武汉本地抽样。GA 前补一轮沿海（上海 / 广州）+
   北方（北京）样本，避免武汉代全国偏差。
