...
> ⚠️ **本节 2026-05-12 起部分失效，以 §16 为准**：production 现已存在 `pricing_runtime.json`（2026-05-08 写入），且 2026-05-12 加 `smart.standard:100`。但 `cost_model` 字段仍是 code defaults（per 项目 owner 同意，P2 试运行后再 calibrate），所以本节下方对 cost_model 默认值的描述仍有参考价值。
...
## 16. 2026-05-12 增补：基础设施完工 + 自然累积进展（fresh snapshot）

> **本节是最新真实状态。**前面 §1-§15 部分数字已过时（§15 的 12/20 是 2026-05-10 frozen 数字；§2.3 关于 pricing_runtime.json 不存在的描述已失效）。任何 audit / 决策依据应以本节为准。

### 16.1 P2 launch 阻塞地图（实测，2026-05-12）

| 阻塞 | §15（2026-05-10） | §16（2026-05-12） | 状态 |
|---|---|---|---|
| `post_phase_metered_jobs ≥ 10`（P0 重跑门槛） | 12 ✅ | 17 ✅✅ | 远超门槛 |
| `post_phase_metered_jobs ≥ 20`（P2 launch） | 12/20 | **17/20** | 缺 3 |
| §11 verdict | INCONCLUSIVE → PASS | **PASS** | 数据增多 confidence 更高 |
| Production `pricing_runtime.json` | ❌（§2.3 caveat） | **✅ 2026-05-08 已存在 + 2026-05-12 加 smart.standard:100** | 文件 in place |
| Bug 1（emitter marked_event_ids） | 未提及 | **✅ in main + deployed**（`416e08e` + `f99edf7`） | 完成 |
| Follow-up #1（split shim） | 未提及 | **✅ in main + deployed**（`6344b5d`） | 完成 |
| Follow-up #2（collector 两步读） | 未提及 | **✅ in main**（`5c88213`） | 完成（本地工具） |
| Phase D Whisper 设计正确性 | 未提及 | **✅ 已确认 deliverable trigger 是 by-design** | 不改 |
| `cost_model` 真实账单 calibration | (隐含阻塞) | **降级为非阻塞** | per 项目 owner 同意，P2 试运行后再调整 credits/min |

**真实剩余硬阻塞：post_phase_metered_jobs 17 → 20（差 3，按 1.2 jobs/天 节奏约 3 天）。**

### 16.2 2026-05-07 之后 audit / pricing 基础设施改进（in main）

| Commit | 内容 | 影响 |
|---|---|---|
| `416e08e` | fix: effective_marker.marked_event_ids no longer always empty | Bug 1 emitter 修复，post-edit commit 后 marker 非空 |
| `f99edf7` | test: marked_event_ids survivor logic + emit-path integration | TDD 配套 |
| `6344b5d` | fix: split audit shim reads new_segments not new_segment_ids | Follow-up #1，split event 后 child_segment_ids 正确填充 |
| `5c88213` | fix: collector two-pass reads marked_event_ids from effective_marker events | Follow-up #2，collector 消费 marker 端正确 |
| `505c736` | fix(smart-shadow): post_phase_metered_jobs gate uses drift_count not alignment_model | Gate fix，post_phase 计数从误漏 Phase A/B-only jobs 恢复 |
| `06dbf54` | docs(smart-shadow): record gate fix + §11 PASS | §15 doc sync |

外加 2026-05-12 直接写 production 文件（不走 git）：
- `/opt/aivideotrans/config/pricing_runtime.json` 加 `smart.standard: 100`（备份 `/tmp/pricing_runtime.bak.1778575840.json`）

### 16.3 当前 production pricing_runtime.json 状态
...
```json
{
  ...
  "cost_model": { /* 仍 code defaults，per owner 同意延后 calibration */ }
}
```

Gateway 跨 process cache 通过 mtime 监听自动失效，新值生效**无需 restart**。

### 16.4 fresh 数字复用入口

[`D:/Claude/temp/smart_shadow_sim/buffett_spot_check/scan_recent.py`](../../../temp/smart_shadow_sim/buffett_spot_check/scan_recent.py) 可重跑获取最新 production 真实数字（per-job table + aggregate）。任何 audit 不要直接信本文档 frozen 数字，重跑这个再说。

c7 远端只读 collector + simulator + aggregator 重跑入口：[`D:/Claude/temp/smart_shadow_sim/c7_remote_run/run_remote.py`](../../../temp/smart_shadow_sim/c7_remote_run/run_remote.py)（无需付费 API，0 字节 project_dir 触本地）。

### 16.5 验证 fixes 全 LIVE（2026-05-12 实测）

| Fix | 验证维度 | 数据 |
|---|---|---|
| **Follow-up #1** | 18/18 splits 全有 child_segment_ids | 100% ✓ |
| **Bug 1** | post-deploy job 的 marker 非空 | 7 jobs 有 marker，总 30/42 marked_ids（71.4% 有效率，含 chained edits 的 survivor 过滤） |
| **Phase D Whisper** | jianying=succeeded 任务 → alignment_model="small" | 11/11 ✓，平均 97% whisper-aligned |
| **Gate fix** | post_phase_metered = drift+metering，非 alignment_model | 17/18（vs 旧 gate 11/18） |

### 16.6 18 jobs 内容多样性（2026-05-04 之后）

- 多数 1-2 speaker 访谈/讲解（Buffett / Munger / CNBC 风格）
- edit_generation 分布：0 (12), 1 (2), 2 (3), 4 (1) — 多数任务没经过 post-edit
- ⚠️ **未覆盖**：3 人讨论、>30min 长视频、高背景噪音/音乐场景
- P2 launch 前建议主动跑 1-2 个边界 case，避免 launch 后第一次遇到出问题

### 16.7 进 P2 真实路径估算

| 阶段 | 估时 |
|---|---|
| 等 3 个新 metered jobs 自然累积 | ~3 天 |
| （可选）跑 1-2 个边界 case 内容 | 1-2 天 |
| 跑最终 c7 重跑确认 §11 verdict 稳定 PASS | 半天 |
| **数据完全就绪决策点** | **~1 周内** |
| P2 实施编码（service_mode=smart / consent / decision layer / Smart QA report / kill switch） | 3-5 周 |
| 灰度上线 + 监控 | 1-2 周 |
| **正式开放用户** | **~5-7 周后（2026-06 下旬到 7 月初）** |

### 16.8 给 audit 读者的建议

如果你在 read this doc 做 P2 launch readiness audit：

1. **不要直接信 §15 之前的 frozen 数字**——读 §16，或重跑 [scan_recent.py](../../../temp/smart_shadow_sim/buffett_spot_check/scan_recent.py)
2. **不要把 cost_model code defaults 列为 launch blocker**——owner 已明确接受先 launch 再 calibrate
3. **Bug 1 / Follow-up #1 / Follow-up #2 / Gate fix 都 in main**——audit 时不要把它们列为 pending
4. **Pricing snapshot 文件 in place**——只是 cost_model 未 calibrate，credits.debit_rates 已含 smart.standard:100
