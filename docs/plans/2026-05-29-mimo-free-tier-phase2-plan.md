...
> ✅ **Task 7 完成**（本批提交）。卡口落在**流水线入口**——`process._check_duration_limit` 在 `_ffprobe_duration_ms`(可信 ffprobe，process.py:2768) 之后、ASR transcribe 之前，新增 `service_mode == "free"` 分支：
> - 纯决策抽到 `utils/free_duration_gate.evaluate_free_duration_cap`(轻量可单测，不拉 process.py)：`>10min` → `REJECT_OVER_CAP`；**duration ≤ 0 / None / 非有限(NaN/inf) / 非数 → `REJECT_UNTRUSTED`（FAIL-CLOSED，CodeX P2 补 `math.isfinite`）**；恰好 10:00 放行(strict `>`)。
> - free 分支两 reason 都 `raise RuntimeError`(升级/重传提示)→ 任务在 ASR/LLM/TTS **之前**失败；URL+上传两路都经此点(ffprobe 后)，故两路都覆盖。
> - 付费模式维持现状(plan-based、unknown=0 宽松放行)不变。
> - Gateway 创建期既有 `duration_limit` 检查(plan cap、yt-dlp 探测)保留作 URL 早拒的 best-effort 层；上传无法在 gateway ffprobe，由流水线入口的 fail-closed 闸 authoritative 兜底。
>
> `11 passed`(纯决策 over/under/边界/zero/neg/None/非数/custom-cap + 两条 process.py wiring 源码守卫)。process.py AST 校验通过。

- [x] **Step 1:** 失败测试：`mode=free` ffprobe>10min → ASR/LLM/TTS 前拒绝（源码守卫 + 纯决策单测）。
- [x] **Step 2:** fail-closed：`mode=free` 拿不到可信时长(0/None/非数/探测失败)→ 拒绝/要求重传，不进 ASR/LLM/TTS；付费模式不变。
- [x] **Step 3:** 实现（`utils/free_duration_gate` + `_check_duration_limit` free 分支 + run() 传 `service_mode_snapshot`）+ 测试 + Commit。

## Task 8（§4.2）：免费版水印（最小实现）

**Files:** 发布阶段 publish 代码、tests

> ✅ **Task 8 完成**（本批提交）。free 服务的 `publish.dubbed_video` 经 ffmpeg `drawtext` 烧入水印,付费版保持 `-c:v copy` 干净输出。数据流:`run()` → `free_watermark_text_for(job_service_mode)`(free→文本/其它→None) → `_dispatch_process_output_bundle(watermark_text=)` → `OutputRequest.watermark_text` → `OutputDispatcher` → `PublishRequest.watermark_text` → `VideoRenderer._build_render_command`。
> - 新 `utils/free_watermark`:`FREE_WATERMARK_TEXT`(env 可覆盖,ASCII 默认)、`WATERMARK_FONTFILE`(env)、`build_drawtext_filter`(右下角带框,`x=w-tw-20/y=h-th-20`)、`free_watermark_text_for`。
> - `VideoRenderer._build_render_command`:有水印 → `[0:v]drawtext…[vout]` + **`-c:v libx264` 重编码**(drawtext 与 `-c:v copy` 不兼容);无水印 → 维持 `-c:v copy`。三轨(ambient)/双轨两路都覆盖,ambient amix 保留。
> - γ resume_publish_only(Studio 后编辑 republish)call site 保持 `watermark_text=None`(付费,不加)。
> - ⚠️ **部署依赖**:`drawtext` 需容器内可用字体。`AVT_WATERMARK_FONTFILE` 指向 `.ttf`(libfreetype 直读,免 fontconfig);CJK 水印文本需 CJK 字体。验收时确认字体存在(否则 free publish 会 `PublishError`)。
>
> `11 passed`(policy/filter/命令构建 双轨×水印开关/模型字段/wiring 守卫)+ 回归 `test_publish_backend`/`test_output_dispatcher`/`test_video_render_async` 共 `30 passed`。process.py AST OK。

- [x] **Step 1:** 失败测试:`mode=free` 的 `publish.dubbed_video` 经 `drawtext` 加水印、付费不加(命令构建单测 + wiring 守卫)。
- [x] **Step 2:** 实现 drawtext 叠加(文字/位置/字体走 `utils/free_watermark` 配置默认值 + env 覆盖;admin UI 留 Phase 2b)。
- [x] **Step 3:** 测试 + Commit。
