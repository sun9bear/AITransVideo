...
- [ ] **Step 1（纯函数先行）:** `shanghai_day_key(now=None)` → Asia/Shanghai 自然日 `YYYY-MM-DD`（镜像 `user_voice_service` 的 `ZoneInfo("Asia/Shanghai")` + `timezone(+8)` fallback）。失败测试 → 通过。
- [ ] **Step 2（model + migration）:** `FreeServiceDailyUsage`（镜像 `ExpressCloneReservation`）：`id` / `user_id`(FK CASCADE) / `usage_date`(SH day str) / `create_idempotency_key`(唯一→幂等) / `job_id`(nullable) / `status`(reserved/consumed/released) / `created_at` / `updated_at`；migration `034` 链 `033`，建表 + 唯一约束 + `(user_id, usage_date)` 查询索引。model/migration 测试。
- [ ] **Step 3（reserve service）:** `free_service_quota.reserve_free_daily(db, user_id, idempotency_key)` 单事务锁 `users` row → 数 active 行 → `>= cap` 拒绝；否则插 `reserved` 行。`consume_free_daily`(upstream 接受) / `release_free_daily`(upstream 失败)。失败测试：并发只准入 cap 个；同 idempotency_key 重复不双占。
- [ ] **Step 4（wiring）:** `intercept_create_job` free 分支：**upstream forward 前** `reserve_free_daily`，拒绝 → 403 `free_daily_quota_exceeded` + 升级提示；upstream 接受 → `consume`，失败 → `release`；**且 `service_mode=="free"` 跳过 legacy `reserve_quota`**。失败测试：free job 创建后 daily ledger +1 但 `user.free_jobs_quota_used` 不变；旧 express/free-plan job 仍走 legacy quota。
- [ ] **Step 5:** 回归 + Commit。
