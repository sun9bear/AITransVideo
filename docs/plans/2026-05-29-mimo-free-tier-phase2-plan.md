...
> - 纯决策抽到 `utils/free_duration_gate.evaluate_free_duration_cap`(轻量可单测，不拉 process.py)：`>10min` → `REJECT_OVER_CAP`；**duration ≤ 0 / None / 非数 → `REJECT_UNTRUSTED`（FAIL-CLOSED）**；恰好 10:00 放行(strict `>`)。
