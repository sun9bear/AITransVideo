...
字段命名守卫 + ORM 一致性守卫合在一个文件：
``tests/test_alembic_031_user_voice_temp_expiry.py``
AST 扫 migration + ORM 反射两端字段，类型 / nullable / server_default /
partial index where 子句全部对齐；3 条命名守卫锁死禁止裸 ``expires_at``
和禁止用 ``temporary_expires_at`` 做 active 判断（含 SQLAlchemy Compare /
Call AST 形态）。
